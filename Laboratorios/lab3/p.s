...
reset:
    ...
    LDR R1,=GPIO_CLR0
    LDR R0,=LED_MASK //Me dice 1 donde tengo led
    ...
    // Se configuran los bits gpio del led 1 como salidas
    LDR R0,=LED_1_MASK
    STR R0,[R1,#LED_1_OFFSET]

    // Configura los bits gpio de los leds 2 Y 3 como salidas
    LDR R0,[R1,#LED_OFFSET]
    LDR R2,=LED_MASK
    ORR R0,R2
    STR R0,[R1,#LED_OFFSET]
...
refrescar:
    ...
    ORREQ R3,#(1 << LED_1_BIT)
    STR   R3,[R4,#(LED_1_GPIO << 2)]
...
systick_isr:
    ...
    STR R0,[R1,#LED_N_OFFSET] // Se invierte el bit GPIO del LED 1
